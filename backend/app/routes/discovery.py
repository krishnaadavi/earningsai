...
from app.services.guidance_enricher import enrich_guidance_for_doc
    ...
    try:
        ...
        try:
            if is_db_enabled():
                ...
                # Create highlight + ensure event for dashboard parity if ticker provided
                try:
                    if req.ticker:
                        create_highlight_and_event(
        ...
        # Kick off guidance enrichment asynchronously (best effort)
        try:
            asyncio.create_task(enrich_guidance_for_doc(doc_id, doc=store.documents.get(doc_id)))
        except Exception:
            pass
        ...
        async with httpx.AsyncClient(timeout=20.0) as client:
            ...
            if r.status_code < 400:
                ...
                for it in items:
                    ...
                    if isinstance(link, str) and (link.lower().endswith('.pdf') or 'pdf' in link.lower()):
                        return link
...
async def ingest_symbol(req: IngestSymbolRequest) -> UploadResponse:
    ...
    # Download the candidate URL (PDF or HTML). We allow HTML to support SEC exhibit pages.
    ...
    try:
        # Decide parser based on content-type or URL extension
        is_html = bool((ctype or "").startswith("text/html") or (pdf_url or "").lower().endswith((".htm", ".html")))
        pages = extract_pages_from_html(data) if is_html else extract_pages_from_pdf(data)
        chunks = chunk_pages(pages)
        texts = [c.text for c in chunks]
        embs = embed_texts(texts)
